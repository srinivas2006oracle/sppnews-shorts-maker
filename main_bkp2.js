...
app.post('/upload', upload.array('media', 10), async (req, res) => {
  ...
  for (let i = 0; i < mediaFiles.length; i++) {
    ...
    // Use ffmpeg filter to speed up video and set background color
    await new Promise((resolve, reject) => {
      ffmpeg(file.path)
        .videoCodec('libx264')
        .audioCodec('aac')
        .outputOptions([
          `-vf scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2:color=#2d5072,setsar=1,setpts=PTS/1`,
          '-pix_fmt yuv420p',
          '-shortest'
        ])
