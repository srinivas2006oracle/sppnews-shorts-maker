...
const gTTS = require('node-gtts')('te-IN');// Initialize gTTS for English language
const app = express();
...
// Function to generate AI audio using node-gtts
function generateAIAudio(text, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    writeStream.on('finish', () => {
      console.log(`AI audio generated successfully: ${outputPath}`);
      resolve(outputPath);
    });
...
app.post('/upload', upload.array('media', 10), async (req, res) => {
  ...
  const audioPath = path.join(tempDir, 'ai-bg-music.mp3');
  const spedUpAudioPath = path.join(tempDir, 'ai-bg-music-sped-up.mp3');
  ...
  // Speed up the audio
  await speedUpAudio(audioPath, spedUpAudioPath);
  ...
  for (let i = 0; i < mediaFiles.length; i++) {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .input(topBannerPath)
        .complexFilter(`
          [0:v]scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2:color=#2d5072,setsar=1,setpts=1.1*PTS[spedv];
